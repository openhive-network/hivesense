...
query_and_link AS (
    SELECT
        npid.query AS query,
        (npid.nearest_post_id).similarity_order AS similarity_order,
        'https://hive.blog/@' || ha.name || '/' || hpd.permlink AS link
    FROM nearest_posts_id AS npid
    INNER JOIN hivemind_app.hive_posts AS hp ON hp.id = (npid.nearest_post_id).post_id
    INNER JOIN hivemind_app.hive_accounts AS ha ON hp.author_id = ha.id
    INNER JOIN hivemind_app.hive_permlink_data AS hpd ON hp.permlink_id = hpd.id
)

SELECT JSONB_AGG(
    qal.*
    ORDER BY qal.similarity_order
) FROM query_and_link AS qal;
