...
docker build -t "registry.gitlab.syncad.com/hive/hivesense/postgrest-rewiter:${TAG}" -f "${SCRIPTPATH}/../Dockerfile.rewriter"  "${SCRIPTPATH}/.."
...
if [ -n "${PUSH:-}" ]; then
  ...
  docker push "registry.gitlab.syncad.com/hive/hivesense/postgrest-rewiter:${TAG}"
